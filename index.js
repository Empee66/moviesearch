...
function searchMovies() {
  ...
  if (!searchTerm || /[<>{}"'();]/g.test(searchTerm) || searchTerm.includes('script')) {
...
async function showMovieDetails(movie) {
  showLoading(true);
  try {
    console.log(`Fetching details: ${BASE_URL}?i=${movie.imdbID}&apikey=${API_KEY}`); 
    ...
    document.getElementById('modalRelease').textContent = `Release Date: ${data.Released || 'N/A'}`;
    document.getElementById('modalRating').textContent = `Rating: ${data.imdbRating || 'N/A'}/10`;
    document.getElementById('modalOverview').textContent = `Overview: ${data.Plot || 'No overview available'}`;
